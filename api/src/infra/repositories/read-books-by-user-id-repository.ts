...
export class ReadBooksByUserIdRepository implements IReadBooksByUserIdRepository{
  async read(userId: string): Promise<Book[] | null> {
    ...
    await DataSourceOperation(async (DataSource)=>{
      ...
      const userData: UserEntity | null = await userRepository.findOne({where: {
        id: userId
      }, relations: ["books"] })
      if(!userData){
        return
      ...
      userData.books.map(bookData => {
        if(!books){//This if shouldnt exist
          books = []
        }
        books.push({
          id: bookData.id,
